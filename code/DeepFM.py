'''
Tensorflow implementation of Deep Factorization Machines (DeepFM)
...
def parse_args():
    ...
    parser.add_argument('--keep_prob', nargs='?', default='[1,1,1]', 
                        help='Keep probility (1-dropout) of each layer. 1: no dropout. The last index is for the bilinear interaction layer.')
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print("DeepFM: dataset=%s, factors=%d, dropout_keep=%s, layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d" 
    ...
    # Training
    t1 = time()
    model = DeepFM(data.features_M, args.hidden_factor, eval(args.layers), args.loss_type, args.pretrain, args.epoch, args.batch_size, args.lr, args.lamda, eval(args.keep_prob), args.optimizer, args.batch_norm, activation_function, args.verbose, args.early_stop)
